	...
	*,
	...
	for event in events:
		...
		if mode in {"updates", "values", "debug", "tasks"}:
			...
			# normalize_stream_event json-readies the event, so a ("messages",
			# (chunk, meta)) tuple arrives here as a [chunk, meta] list. Unwrap
			# either form to recover the chunk payload.
